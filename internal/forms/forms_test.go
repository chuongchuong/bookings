...
func TestForm_Has(t *testing.T) {
	...
	has = form.Has("a")
	if !has {
		t.Error("shows form does not have field when it does not")
	}
}
...
func TestForm_MinLength(t *testing.T) {
	...
	if !form.Valid() {
		t.Error("shows minlength of 1 when it is")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	postedValues = url.Values{}
	postedValues.Add("email", "abc123@ga.com")
	form = New(postedValues)

	form.IsEmail("email")
	if !form.Valid() {
		t.Error("got an invalid email")
	}
